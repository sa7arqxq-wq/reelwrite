...
import {
  validateHook, validateCaption, validateMood,
  validateBookTitle, validateGenre, validateUrl, validateId, sanitizeText,
} from "@/lib/validation";
...
export async function POST(req: NextRequest) {
  ...
  if (!allowed) return NextResponse.json({ error: reason, upgradeRequired: true, subscription: info }, { status: 402 });
  ...
  const reel = await db.reel.create({
    data: {
      ...
      backgroundImage: actualBackground === "image" && typeof backgroundImage === "string" && backgroundImage.length < 20_000_000
        ? (backgroundImage.startsWith("data:image/") || backgroundImage.startsWith("https://") ? backgroundImage : null) : null,
      videoUrl: actualBackground === "video" && typeof videoUrl === "string" && videoUrl.length < 25_000_000
